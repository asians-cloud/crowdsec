...
func (c *Controller) AddBouncer(gctx *gin.Context) {
	...
	hashedKey := middlewaresv1.HashSHA512(bouncerKey)

	bouncer, err := c.DBClient.CreateBouncer(ctx, bouncerName, gctx.ClientIP(), hashedKey, types.ApiKeyAuthType, false)
	if err != nil {
...
func (c *Controller) DeleteBouncer(gctx *gin.Context) {
	...
	if err := c.DBClient.DeleteBouncerAllowMissing(ctx, bouncerName); err != nil {
